...
- **分析**：各情景按其权重**加权平均风速**、**静风频率**（v<1.5 m/s）、**强风频率**（v>判决阈值 6.2 m/s）→ 停放适宜性分级（🔴高风险 / 🟠中风险 / 🟢适宜 / 🔵静风区）
  > 判决阈值 = v_crit × gust_factor = 11.7 × 0.53 ≈ 6.2 m/s。
  > v_crit=11.7 m/s 是**阵风口径**的倾覆临界值，而模拟/上传数据是**平均风速**，
  > 故按 G = 1 + k·TI = 1 + 3×0.3 ≈ 1.9 换算（推导见 `frontend/evaluator.py` 文件头）。
