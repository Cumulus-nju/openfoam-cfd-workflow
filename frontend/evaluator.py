...
# ── gust_factor 的口径（2026-09-22 已由用户确认，别再改错方向）─────────────
# 用户原话：「11.7 当然是阵风风速（使单车倾倒），只是我们模拟的是平均风，所以要乘系数」
#
# 即：阵风 > 平均风（G = 阵风/平均 > 1，城市下垫面常用 1.5~2.0）。
#     V_CRIT = 11.7 是【阵风】口径的倾覆临界值；CFD/上传数据是【平均】风速。
#     所以「平均风速达多少就危险」= V_CRIT / G ≈ 11.7 / 1.49 ≈ 7.84 m/s。
#     代码写作 v_eff = v_crit * gust_factor = 11.7 * 0.67 = 7.84 m/s，
#     即 **gust_factor 里存的是 1/G（<1）**，G ≈ 1.49。方向正确，不要反着改。
#
# ⚠ 旧注释写法 "阵风修正 (7.8/11.7)" 字面是 7.8÷11.7，容易被读成
#   「7.8 是阵风、11.7 是平均」——那是反的。正确关系：11.7 是阵风口径，
#   7.8 是换算到平均口径后的判决阈值。
# ⚠ 若哪天换成【平均】口径的临界值，才该取 gust_factor = 1.0（不折减）。
...
CONTEXTS: Dict[str, Dict[str, Any]] = {
    ...
    "drone": {
        ...
        # ⚠ 这里**不套用单车的 0.67**：单车的 0.67 是因为 V_CRIT=11.7 是【阵风】口径、
        #   而模拟给的是平均风，才需要做「阵风→平均」换算（用户已确认该口径）。
        #   厂商标称的抗风等级本身就是一个风速值，没有同等的换算依据，
        #   所以 gust_factor 取 1.0（判决阈值 = 12.0）。
        #   若你机型标称值指的是**阵风上限**，再按 G 换算（即 gust_factor 改 1/G）。
        "v_crit": 12.0,
        "gust_factor": 1.0,
        "high_factor": 0.85,
        "medium_factor": 0.55,
